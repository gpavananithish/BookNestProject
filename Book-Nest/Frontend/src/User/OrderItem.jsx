  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const updatedFormData = {
        ...
        Delivery: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toLocaleDateString(),
      };
    ...
    <div style={{ backgroundColor: '#f8f9fa' }}>
      ...
      <div style={{ display: 'flex ' }} >
        <div className="max-w-md mx-auto mt-8 p-4 border rounded shadow-lg bg-white">
          <h2 className="text-2xl font-semibold text-center" style={{ color: "darkcyan" }}>Your order is almost Done! </h2>
            ...
            {item && (
              <div>
                ...
                <div style={{ display: 'flex', justifyContent: "space-between" }}>
                  ...
                  <p> {parseInt(item.price, 10) + 99}</p>
                </div>
              </div>
            )}
            <button
              type="submit"
              style={{ width: "340px", backgroundColor: "darkcyan", borderColor: "darkcyan" }}
              className=" hover:bg-blue-800 text-white font-semibold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
            >
              Order
            </button>
          </form>
